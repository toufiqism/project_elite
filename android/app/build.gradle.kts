...
// See android/key.properties.example for the expected shape. If the file is
// missing the release build falls back to debug-signing so local builds still
// work — but Play Store will reject those AABs.
...
android {
    namespace = "com.toufiq.elite.project_elite"
    ...
    defaultConfig {
        ...
        applicationId = "com.toufiq.elite.project_elite"
    ...
    signingConfigs {
        if (hasReleaseSigning) {
            create("release") {
                ...
                // Resolve storeFile relative to the android/ root (where
                // key.properties lives), not android/app/ — that's the path the
                // user typed and where keytool wrote the .jks.
    ...
    buildTypes {
        release {
            signingConfig = if (hasReleaseSigning) {
                ...
                // Fallback so `flutter run --release` works without a keystore.
